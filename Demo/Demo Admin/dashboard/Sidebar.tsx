import {
  ...
  BarChart2, Settings, Ticket, Handshake, Gift,
  LogOut, X, Store,
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'partnerships', icon: Handshake, labelKey: 'partnerships', soon: true },
      { id: 'occasions', icon: Gift, labelKey: 'occasions' },
    ],
  },
]

export default function Sidebar() {
  const { activeNav, setActiveNav, tr, lang, isSidebarOpen, setIsSidebarOpen } = useDashboard()
      ...
      >
        ...
        <div className="px-5 py-5 border-b border-[rgb(244_242_245)] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-bold text-[rgb(60_28_84)] text-sm leading-none">{store.store_name}</p>
              <p className="text-[10px] text-[rgb(60_28_84)]/50 mt-0.5">{store.slug}</p>
            </div>
          </div>
          <button
            onClick={() => setIsSidebarOpen(false)}
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1 text-start">{tr[item.labelKey] as string}</span>
                      {item.soon && (
        ...
        <div className="px-3 pb-4 pt-3 border-t border-[rgb(244_242_245)]">
          <div className="flex items-center gap-3 px-3 py-3 mb-1 rounded-xl hover:bg-[rgb(244_242_245)] transition-colors cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[rgb(60_28_84)] truncate">{store.admin_name}</p>
              <p className="text-[11px] text-[rgb(60_28_84)]/50 truncate">{store.admin_email}</p>
            </div>
          </div>
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-red-500 hover:bg-red-50 transition-all font-medium mt-1">
